...
// Create a candidate
router.post('/candidate', ({ body }, res) => { // HTTP request method post() to insert a candidate into the candidates table.
...
router.get('/candidate/:id', (req, res) => { // the endpoint has a route parameter that will hold the value of the id to specify which candidate we'll select from the database.
    const sql = `SELECT candidates.*, parties.name 
                 AS party_name 
                 FROM candidates 
                 LEFT JOIN parties 
